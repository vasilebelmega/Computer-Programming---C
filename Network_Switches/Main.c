...
void show(FILE* output, ALG Stive, ALG ACoada, ALG ACoadaLast, int nrStive) 
{
	TSwitch* entry, *entryTemp, *entryTemp1;
	TLG pStiva, CoadaTemp, CoadaTempLast, StivaTemp = NULL;
	int index = 0;
	...
	*ACoadaLast = CoadaTempLast;
	...
	for (index = 0; index < nrStive; index++)
	{
		fprintf(output, "%d:\n", index);
		pStiva = Stive[index];
		while (Stive[index] != NULL)
		{
			entryTemp = (TSwitch*)((Stive[index])->info);
			fprintf(output, "%d %s %s\n", entryTemp->id, entryTemp->ip, entryTemp->nume);
			InsLG(&StivaTemp, (TSwitch*)entryTemp, sizeof(TSwitch));
			DistrugeLG(&Stive[index]);
		}
		// rebuild the new stack without the switch with id "id"
		while (StivaTemp != NULL)
		{
			entryTemp = (TSwitch*)(StivaTemp->info);
			InsLG(&Stive[index], (TSwitch*)entryTemp, sizeof(TSwitch));
			DistrugeLG(&StivaTemp);
		}
	}
	fprintf(output, "\n");
...
int main(int argc, char** argv)
{
	...
	while (fscanf(input, "%s", buffer) != EOF)
	{
		...
		{
			show(output, Stive, &Coada, &CoadaLast, nrStive);
		}
